...
const CursorController = () => {
  ...
  const current = models[activeModel]?.current;
  const isFullScreen = models[activeModel]?.isFullScreen;
  const items = models[activeModel]?.items[current];

  useEffect(() => {
    const cursor = `<svg style="transform:translate(-23px,-20px) scale(0.7);transition:0.5s" width="88" height="88" version="1" xmlns="http://www.w3.org/2000/svg">
